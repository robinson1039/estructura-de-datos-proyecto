...
const EstudiantesIngenieria = () => {
  ...
  const fetchEstudents = async () => {
    try {
      ...
      setResultado([]); // Limpiar el estado de búsqueda para mostrar todos los estudiantes
    } catch (error) {
            ...
            {resultado.length > 0 || Estudents.length > 0 ? (
              (resultado.length > 0 ? resultado : Estudents).map((student) => (
                <tr key={student.id}>
                  ...
                  <td className="px-4 py-2">{student.semestre_actual}</td>
                  <td className="px-4 py-2">{student.promedio_acumulado}</td>
                  <td className="px-4 py-2">{student.serial}</td>
